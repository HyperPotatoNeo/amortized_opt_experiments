...
def main():
    ...
    X = torch.as_tensor(data[:, :4]).cuda()
    Y = (X[:, 2].cos() > 0.98).reshape(-1, 1).float()
    # Y *= (abs(X[:, 0]) < 1.0).reshape(-1, 1)

    training = random.sample(range(len(X)), int(0.8 * len(X)))
    validation = list(set(range(len(X))) - set(training))

    train_x = (X[training] - X.mean(axis=0)) / X.std(axis=0)
    train_y = (Y[training] - Y.mean(axis=0)) / Y.std(axis=0)

    validation_x = (X[validation] - X.mean(axis=0)) / X.std(axis=0)
    validation_y = (Y[validation] - Y.mean(axis=0)) / Y.std(axis=0)
    ...
    torch.save(model, 'rewards_models/rm_2.zip')
